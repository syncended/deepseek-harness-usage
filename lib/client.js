...
`;
    ...
    function sessionRows(sessions, query, grouping, sort, direction, timeZone) {
      ...
      if (grouping !== "none") {
        ...
        rows = [...groups.values()].map((group) => ({ ...group, routes: [...new Set(group.routes)] }));
      }
      const compare = (a, b) => {
    ...
    function SessionTable({ sessions = [], timeZone = "UTC" }) {
      ...
      const renderRow = (row, child = false) => {
        ...
        const unpriced = row.totalTokens > (row.pricedTokens || 0);
        ...
        return h(React.Fragment, { key: group ? "group:" + row.key : "session:" + row.sessionId },
          h("tr", { className: child ? "dsh-usage-session-member" : undefined },
            ...
            h("td", { className: "dsh-usage-num" }, row.pricedTokens || !row.totalTokens ? formatCost(row.cost) : null,
              unpriced ? h("span", { className: "dsh-usage-session-meta dsh-usage-unpriced" }, row.pricedTokens ? `${formatExact(row.totalTokens - row.pricedTokens)} tokens unpriced` : "UNPRICED") : null),
          ),
          open ? row.members.map((member) => renderRow(member, true)) : null,
        );
      };
      return h("article", { className: "dsh-usage-panel" },
        h("header", { className: "dsh-usage-panel-head" }, h("div", null, h("h2", { className: "dsh-usage-panel-title" }, "Sessions"), h("p", { className: "dsh-usage-panel-sub" }, "Tokens and estimated cost in the selected range. Groups include matching sessions only. Custom providers use public model list prices unless configured; estimates may differ from your bill."))),
        h("div", { className: "dsh-usage-session-controls" },
          ...
          h("label", null, "Sort by", h("select", { value: sort, onChange: (event) => { setSort(event.target.value); setDirection(event.target.value === "title" ? "asc" : "desc"); } }, h("option", { value: "cost" }, "Cost"), h("option", { value: "totalTokens" }, "Tokens"), h("option", { value: "calls" }, "Calls"), h("option", { value: "title" }, "Title"))),
        ...
        rows.length ? h("div", { className: "dsh-usage-session-scroll", role: "region", "aria-label": "Session usage table", tabIndex: 0 },
          h("table", { className: "dsh-usage-session-table" }, h("caption", { className: "dsh-usage-session-caption" }, "Session usage · estimated USD cost; unpriced tokens excluded"),
            h("thead", null, h("tr", null, ["Session", "Tokens", "Calls", "Cost (USD)"].map((label, index) => h("th", { key: label, scope: "col", className: index ? "dsh-usage-num" : undefined }, label)))),
